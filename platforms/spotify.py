...
import os
import re
from utils.helpers import sanitize_filename, get_temp_dir, format_size, logger, parse_spotify_url
from config.settings import PREMIUM_EMOJIS

class SpotifyDownloader:
    """Handle Spotify downloads via yt-dlp."""
    ...
    def extract_info(self, url: str) -> dict:
        ...
        try:
            ...
            logger.error(f"Spotify extract error: {e}")
            return None
    ...
    def search(self, query: str, limit: int = 10) -> list:
        """Search Spotify (via YouTube for audio)."""
        import yt_dlp
        
        search_url = f"spsearch:{query}"
        opts = {
            'quiet': True,
            'no_warnings': True,
            'extract_flat': True,
