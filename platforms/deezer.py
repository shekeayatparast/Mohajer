"""Deezer downloader module."""
import os
from utils.helpers import sanitize_filename, get_temp_dir, format_size, logger, parse_deezer_url
from config.settings import PREMIUM_EMOJIS

class DeezerDownloader:
    """Handle Deezer downloads via yt-dlp."""
    
    def __init__(self):
        self.name = "deezer"
        self.display_name = "Deezer"
        self.emoji = "💜"
    
    def extract_info(self, url: str) -> dict:
        """Extract information from Deezer URL."""
        ...
        try:
            with yt_dlp.YoutubeDL(opts) as ydl:
                ...
                return self._parse_info(info, parsed['type'])
        except Exception as e:
            logger.error(f"Deezer extract error: {e}")
            return None
    
    def _parse_info(self, info: dict, media_type: str) -> dict:
        ...
        if media_type in ['playlist', 'album'] or info.get('_type') == 'playlist' or 'entries' in info:
            result['type'] = 'playlist' if media_type == 'playlist' else 'album'
            result['tracks'] = []
    ...
    def download_playlist(self, url: str, quality: str = '320', workdir: str = None,
        ...
        if not info or info['type'] not in ['playlist', 'album']:
            return {'success': False, 'error': 'Invalid playlist/album URL'}
    ...
    def search(self, query: str, limit: int = 10) -> list:
        """Search Deezer (via YouTube for audio)."""
        import yt_dlp
        
        search_url = f"dzsearch:{query}"
        opts = {
            'quiet': True,
            'no_warnings': True,
            'extract_flat': True,
